# Copyright (c) 2026, Shyam Sunder Sahani and contributors
...
class FatoraSettings(Document):
	...
	def validate(self):
		create_payment_gateway("Fatora")
		call_hook_method("payment_gateway_enabled", gateway="Fatora")

	def validate_transaction_currency(self, currency):
		if currency not in self.supported_currencies:
			frappe.throw(
				_(
					"Please select another payment method. Fatora does not support transactions in currency '{0}'"
				).format(currency)
			)
	...
	def get_payment_url(self, **kwargs):
		...
		integration_request = create_request_log(kwargs, service_name="Fatora")
		kwargs.update(dict(order_id=integration_request.name))
		# frappe.msgprint(str(integration_request))

		# return get_url(f"./paytm_checkout?{urlencode(kwargs)}")
		# frappe.local.response["type"] = "redirect"
		# frappe.local.response["location"] = "https://www.google.com"
		# frappe.local.response["redirect_http_status"] = 307
		return get_url(f"./fatora_checkout?{urlencode(kwargs)}")
		# return get_url(f"./fatora_checkout?{urlencode(kwargs)}")
		...
		# 	# return get_url(f"./paytm_checkout?{redirect_url}")

		# 	# return get_url(f"{redirect_url}")

		# 	# return redirect_url
		# 	# frappe.local.response["type"] = "redirect"
		# 	# frappe.local.response["location"] = redirect_url
		# except Exception:
		# 	finalize_request(integration_request.name, "error")
...
@frappe.whitelist(allow_guest=True)
def finalize_request(order_id, transaction_response="success"):
	...
	if transaction_response == "success":
		if transaction_data.reference_doctype and transaction_data.reference_docname:
			...
			try:
				...
				# request.db_set("status", "Completed")fatora_settings = frappe.get_single("Fatora Settings")

				fatora_settings = frappe.get_single("Fatora Settings")

				payment_user = "Administrator"
				if fatora_settings.get("payment_user", False):
					payment_user = fatora_settings.get("payment_user", "")
				frappe.set_user(payment_user)
					# self.set_as_paid()
				custom_redirect_to = frappe.get_doc(
					transaction_data.reference_doctype, transaction_data.reference_docname
				).run_method("on_payment_authorized", "Completed")
	...
	success_url = webshop_settings.payment_success_url
	webshop_redirect = get_url("/orders")
	...
	# frappe.local.response["type"] = "redirect"
	# frappe.local.response["location"] = redirect_url
	return {"location":webshop_redirect}
